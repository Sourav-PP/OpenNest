...
const PsychologistDashboardClientTrendGraph = () => {
  ...
  const fetchRevenueStats = async (filterValue: RevenueFilterType) => {
    try {
      const res = await psychologistApi.getClientTrend(filterValue);
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8">
      <Card className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200 shadow-sm">
        <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 p-0 space-y-4 sm:space-y-0">
          {/* Title */}
          <h2 className="text-3xl font-bold text-gray-800 mb-4 text-start">Client trend</h2>
            ...
            <div className="space-y-2">
              <div>
                ...
                <p className="text-lg sm:text-2xl font-bold text-gray-900">
                  {data.reduce((sum: number, item: any) => sum + (item.uniqueUsers || 0), 0).toLocaleString()}
                </p>
              </div>
            </div>
...
export default PsychologistDashboardClientTrendGraph;

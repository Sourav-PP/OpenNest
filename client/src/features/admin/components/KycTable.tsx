    ...
    const fetchKyc = async () => {
      try {
        ...
        const data = await adminApi.getAllKycDetails({
          ...
          status: (filters.status as 'pending' | 'approved' | 'rejected' | 'all') || undefined,
          limit: itemsPerPage,
          page: currentPage,
        });
        setKyc(data.kycs);
    ...
    },
    {
      ...
      render: (item: IAdminKycDto) => (
        <Link to={`/admin/kyc/${item.psychologistId}`}>
          <button className="bg-admin-extra-light px-4 py-1 rounded-full text-white hover:bg-gray-600">
            view
          </button>
        </Link>
      ),

...
export const server = {
  ...
  post: <T, D>(url: string, data?: D, config?: AxiosRequestConfig): Promise<T> =>
    instance.post<T, AxiosResponse<T>, D>(url, data, config).then(res => res.data),
  put: <T, D>(url: string, data?: D, config?: AxiosRequestConfig): Promise<T> =>
    instance.put<T, AxiosResponse<T>, D>(url, data, config).then(res => res.data),
  patch: <T, D>(url: string, data: D, config?: AxiosRequestConfig): Promise<T> =>
    instance.patch<T, AxiosResponse<T>, D>(url, data, config).then(res => res.data),

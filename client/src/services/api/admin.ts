...
import type { ConsultationStatusFilterType } from '@/constants/Consultation';
...
export const adminApi = {
  ...
  getAllPsychologists: async (params?: IGetAllPsychologistsRequest) =>
    server.get<IGetAllPsychologistResponse>('admin/psychologists', { params }),
  toggleUserStatus: async (userId: string, data: IToggleStatusRequest) =>
    server.patch<BackendResponse, IToggleStatusRequest>(`/admin/users/${userId}/status`, data),
  getAllKycDetails: async (params?: IGetAllKycDetailsRequest) =>
    server.get<IGetAllKycDetailsResponse>('/admin/kyc', { params }),
  getKycDetailsByPsychologistId: async (psychologistId: string) =>
    server.get<IAdminKycDto>(`/admin/kyc/${psychologistId}`),
  approveKyc: async (psychologistId: string) =>
    server.patch<BackendResponse, undefined>(`/admin/kyc/${psychologistId}/approve`, undefined),
  rejectKyc: async (psychologistId: string, reason: string) =>
    server.patch<BackendResponse, { reason: string }>(`/admin/kyc/${psychologistId}/reject`, { reason }),
  ...
  addPlan: async (data: IAddPlanRequestData) =>
    server.post<IAddPlanResponse, IAddPlanRequestData>('/admin/plans', data),
  getAllPlans: async () => server.get<IGetAllPlanResponse>('/admin/plans'),
  getPendingPayouts: async (params?: IGetAllPendingPayoutRequest) =>
    server.get<IGetPendingPayoutResponse>('/admin/payout-requests', { params }),
  ...
  getTopPsychologists: async (limit: number) =>
    server.get<ITopPsychologistResponse>('/admin/top-psychologists', { params: { limit } }),
};

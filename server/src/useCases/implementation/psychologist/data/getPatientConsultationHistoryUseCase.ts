...
export class GetPatientConsultationHistoryUseCase implements IGetPatientConsultationHistoryUseCase {
    ...
    async execute(input: IGetPatientConsultationHistoryRequest): Promise<IGetPatientConsultationHistoryResponse> {
        const { search, sort, page = 1, limit = 10, patientId, psychologistUserId } = input;

        const psychologist = await this._psychologistRepo.findByUserId(psychologistUserId);
        ...
        const consultations = await this._consultationRepo.findPatientHistory(psychologistId, patientId, {
            search,
            sort: finalSort,
            limit,
            skip,
        });
        const mappedConsultations = consultations.map(c => toPatientConsultationHistoryDto(c.consultation, c.patient));

        const totalCount = await this._consultationRepo.countPatientHistory(psychologistId, patientId);

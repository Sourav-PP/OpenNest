...
export class ListWalletTransactionsUseCase implements IListWalletTransactionsUseCase {
    ...
    async execute(walletId: string, page = 1, limit = 10): Promise<{ data: WalletTransaction[]; totalCount: number }> {
        const skip = (page - 1) * limit;

        const transactions = await this._walletRepo.listTransaction(walletId, skip, limit);

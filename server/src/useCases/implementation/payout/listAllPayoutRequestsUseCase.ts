...
export class ListAllPayoutRequestsUseCase implements IListAllPayoutRequestsUseCase
{
    ...
    async execute(input: IGetAllPendingPayoutRequest): Promise<IGetAllPendingPayoutResponse> {
        const { search, sort, page = 1, limit = 10 } = input;
        const finalSort = (sort === 'asc' || sort === 'desc') ? sort : 'desc';
        const skip = (page - 1) * limit;

        const requests = await this._payoutRequestRepository.findAllWithPsychologist({
            search,
            sort: finalSort,
            limit,
            skip,
        });
        const mapped = requests.map(r => toPayoutRequestListDto(r.payoutRequest, r.psychologist));
        const totalCount = await this._payoutRequestRepository.countAll({ search });
